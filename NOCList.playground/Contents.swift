...
let agents = [agentOne, agentTwo, agentThree, agentFour, agentFive, agentSix, agentSeven, agentEight, agentNine, agentTen, agentEleven ]
...
func findCleanAgents() -> Array<Any> {
    var uncompromisedAgents: Array<(coverName: String, realName: String, accessLevel: Int, compromised: Bool)> = []
    for agent in agents {
        if !agent.compromised {
...
func checkLevel() {
    ...
    print("\(low) low level agents, \(mid) level agents \(high) high level agents")
}
